...
function IllustrativeSurface({ project, compact = false }: { project: ReferenceProject; compact?: boolean }) {
  return (
    <div className={`illustrative-ui ui-${project.visual} ${compact ? 'is-compact' : ''}`}>
      <header><b>GS</b><span>نماذج من أعمالنا</span><i /></header>
      <main>
        <div className="ui-copy"><small>{project.family}</small><strong>{project.visual === 'service' ? 'ابدأ من حاجتك' : project.visual === 'commerce' ? 'تفاصيل تستحق الاقتراب' : 'ما يحتاج انتباهك الآن'}</strong><p>{project.statement}</p><button type="button" tabIndex={-1}>شاهد التجربة <ArrowLeft /></button></div>
    ...
    <section id="reference-proof" className="reference-proof" aria-labelledby="proof-title" data-project={active.id}>
      ...
      <div className="proof-theatre">
        <ProjectMedia label={`مشروع ${active.family}`}
          ...
          detail={{ alt: `تفصيل من مشروع ${active.index}`, placeholder: <div className="detail-study"><small>{active.index}</small><strong>{active.focus[1]}</strong><span /></div> }} />
        <div className="project-index" aria-label="اختر مشروعًا">
          {referenceProjects.filter((project) => project.id !== active.id).map((project) => <button key={project.id} type="button" aria-pressed="false" onClick={() => setActiveId(project.id)} onFocus={() => setActiveId(project.id)}><i>{project.index}</i><span>{project.family}</span></button>)}
        </div>
      </div>
      <p className="proof-disclosure">أربعة مشاريع مستقلة، يجمعها اهتمام واحد: تجربة واضحة ومتماسكة.</p>
    </section>
  );
}
